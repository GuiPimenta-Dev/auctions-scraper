...
class FrancoLeiloes(BaseRequests):
    ...
    def __parse_querystring(self):
        ...
        return {"cidade": cidade, "estado": estado, "vendedor": "1", "bairro": "1", "tipo": "1",
                "favoritos": "false", "tipoBusca": "Abertos", "aonde": "btnBuscaTexto", "expressao": ""}

    def parse(self, response):

        divs = response.xpath('//div[@class="col-lg-3 col-md-6 col-sm-6 maxheight2"]').extract()
        ...
        for div in divs:
            ...
            if div.xpath(
                    '//div[@class="ribbon sticky statusAbertoLeiloesList1830 shadow green"]/text()').get() == 'Aberto':
                url = div.xpath('//div/@data-link').get()

                description = self.clean_html_tags_from_string(div.xpath('//div[@class="descLeilaoHome"]/text()').get())

                price = div.xpath('//p[@class="pracas negrito margin-top-5 margin-bottom-4"]/text()').getall()[
                    -1].strip().split(': ')[1]
